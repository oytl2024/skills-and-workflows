...
## Task Re-review Finding

- Important: data coverage treats any nonempty manifest JSON as valid. A newer partial manifest such as `{ "status": "in_progress" }` can still hide an older completed capture, and nonnumeric count fields can crash during `int(...)` conversion.
- Required fix: validate required coverage schema and numeric fields before selecting a manifest; add a regression for a newer partial or invalid manifest.
- Minor: command test should cover `max_datasets_per_scope`, `max_fields_per_dataset`, and `resume_capture` forwarding.

## Task 4 Re-review Fix

- Files changed: `BrainWorkflow/wqb/console_state.py`, `BrainWorkflow/tests/test_console_state.py`, `BrainWorkflow/tests/test_console_jobs.py`.
- Coverage selection now requires numeric-convertible, nonnegative `field_count`, `scope_count`, `data_set_count`, and `error_count`; partial or malformed newer manifests are skipped.
- Added regression coverage for newer partial and malformed manifests, plus forwarding assertions for dataset/field limits and resume capture.
- Test commands:
  - `python -c "import sys, runpy; sys.platform='linux'; runpy.run_module('unittest', run_name='__main__')" tests.test_console_state tests.test_console_jobs -v` -> 16 tests passed.
  - `python -m compileall -q wqb tests` -> passed.
- Commit hash: `80156cef6071b94495dda3fc190bfe7f5a665736`.
